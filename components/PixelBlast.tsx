...
const oklchToRgb = (l: number, c: number, h: number): [number, number, number] => {
    ...
    const cb = c * Math.sin(rad);
    const l_ = l + 0.3963377774 * a + 0.2158037573 * cb;
    const m_ = l - 0.1055613458 * a - 0.0638541728 * cb;
    const s_ = l - 0.0894841775 * a - 1.291485548 * cb;
    const l_val = l_ > 0.008856 ? Math.pow(l_, 3) : (903.3 * l_) / 116;
    const m_val = m_ > 0.008856 ? Math.pow(m_, 3) : (903.3 * m_) / 116;
    const s_val = s_ > 0.008856 ? Math.pow(s_, 3) : (903.3 * s_) / 116;
    const x = 0.4122214708 * l_val + 0.5363325363 * m_val + 0.0514459929 * s_val;
    const y = 0.2119034982 * l_val + 0.6806995451 * m_val + 0.1073969566 * s_val;
    const z = 0.0883024619 * l_val + 0.2817188376 * m_val + 0.6299787005 * s_val;
    ...
    useEffect(() => {
        ...
        if (mustReinit) {
            ...
            const uniforms = {
                ...
                uTime: { value: 0 },
                uColor: { value: new THREE.Color(resolveColor(color)) },
                uClickPos: {
            ...
            if (noiseAmount > 0) {
                ...
                if (composer && composer.passes.length > 0)
                    composer.passes.forEach(
                        (p) =>
                            ((p as unknown as { renderToScreen: boolean }).renderToScreen = false),
                    );
            ...
            const animate = () => {
                ...
                if (liquidEffect)
                    (liquidEffect.uniforms.get('uTime') as THREE.Uniform).value =
            ...
            t.uniforms.uColor.value.set(resolveColor(color));
